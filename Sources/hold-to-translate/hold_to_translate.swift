...
struct TranslationSettings: Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        alternateLanguage = try container.decodeIfPresent(String.self, forKey: .alternateLanguage) ?? "en"
        triggerButton = try container.decodeIfPresent(TriggerButton.self, forKey: .triggerButton) ?? .right
        longPressMilliseconds = try container.decodeIfPresent(Double.self, forKey: .longPressMilliseconds) ?? 450
        thresholds = try container.decodeIfPresent(TranslationThresholds.self, forKey: .thresholds) ?? .init()
    }
}
...
final class SettingsStore {
    ...
    func loadSettings() -> TranslationSettings {
        ...
        return (try? JSONDecoder().decode(TranslationSettings.self, from: data)) ?? TranslationSettings()
    }
...
final class AzureTranslationService: Sendable {
    func translate(text: String, settings: TranslationSettings, apiKey: String, targetLanguageOverride: String? = nil) async throws -> String {
        ...
        let targetLanguage = (targetLanguageOverride ?? settings.targetLanguage).trimmingCharacters(in: .whitespacesAndNewlines)
...
final class AccessibilitySelectionReader {
    func readSelectedText() -> SelectionReadResult {
        ...
        guard let focusedElement = focusedUIElement(from: focusedAppElement, fallback: systemWideElement) else {
    ...
    private func focusedUIElement(from focusedAppElement: AXUIElement, fallback systemWideElement: AXUIElement) -> AXUIElement? {
    ...
    private func supportedAttributeNames(of element: AXUIElement) -> [String] {
        ...
        let attrs = allNames.filter {
            $0 == kAXSelectedTextAttribute as String ||
            $0 == kAXSelectedTextRangeAttribute as String ||
    ...
    private func focusedApplicationElement(from systemWideElement: AXUIElement) -> AXUIElement? {
        ...
        if focusedAppResult == .success, let focusedAppObject, CFGetTypeID(focusedAppObject) == AXUIElementGetTypeID() {
    ...
    private func readSelectedTextFromClipboardFallback() -> String? {
        ...
        while Date() < deadline {
            ...
            if pasteboard.changeCount != clipboardSnapshot.changeCount,
               let selectedText = pasteboard.string(forType: .string)?.trimmingCharacters(in: .whitespacesAndNewlines),
...
final class GlobalMouseHoldMonitor {
    ...
    func start() {
        ...
        if let downMonitor = NSEvent.addGlobalMonitorForEvents(matching: downEvents, handler: { [weak self] event in
            self?.handle(event: event, isMouseDown: true)
        }) {
            monitors.append(downMonitor)
        }

        if let downLocalMonitor = NSEvent.addLocalMonitorForEvents(matching: downEvents, handler: { [weak self] event in
            self?.handle(event: event, isMouseDown: true)
            return event
        }) {
        ...
        if let upMonitor = NSEvent.addGlobalMonitorForEvents(matching: upEvents, handler: { [weak self] event in
            self?.handle(event: event, isMouseDown: false)
        }) {
            monitors.append(upMonitor)
        }

        if let upLocalMonitor = NSEvent.addLocalMonitorForEvents(matching: upEvents, handler: { [weak self] event in
            self?.handle(event: event, isMouseDown: false)
            return event
        }) {
    ...
    private func matchesConfiguredButton(event: NSEvent) -> Bool {
        ...
        case .middle:
            return (event.type == .otherMouseDown || event.type == .otherMouseUp) && event.buttonNumber == 2
        }
    }
...
final class TranslationPanelController: NSWindowController, NSWindowDelegate {
    ...
    private func handleKeyDown(event: NSEvent) -> NSEvent? {
        ...
        let isCommandEnter = (event.keyCode == 36 || event.keyCode == 76) && event.modifierFlags.contains(.command)
...
struct TranslationWindowState: Equatable {
    ...
    mutating func commitEditing() {
        ...
        self.draft = TranslationDraft(originalText: editedText, translatedText: nil, isTranslating: true)
...
struct SearchableLanguageDropdown: View {
    ...
    private var selectedLabel: String {
        options.first(where: { $0.code.caseInsensitiveCompare(selectedCode) == .orderedSame })?.displayLabel ?? selectedCode
    }
    ...
    var body: some View {
        ...
        .popover(isPresented: $isPresented, arrowEdge: .bottom) {
            ScrollViewReader { proxy in
                ...
                .frame(width: 300)
                .onAppear {
                    if highlightedCode == nil || !options.contains(where: { $0.code.caseInsensitiveCompare(highlightedCode ?? "") == .orderedSame }) {
                        highlightedCode = selectedCode
                    }
                    if !filteredOptions.contains(where: { $0.code.caseInsensitiveCompare(highlightedCode ?? "") == .orderedSame }) {
    ...
    private func moveHighlight(direction: MoveCommandDirection) {
        ...
        let currentIndex: Int = {
            if let highlightedCode,
               let idx = filteredOptions.firstIndex(where: { $0.code.caseInsensitiveCompare(highlightedCode) == .orderedSame }) {
                return idx
            }
            return 0
        }()
    ...
    private func commitHighlightedOrFirst() {
        if let highlightedCode,
           let option = filteredOptions.first(where: { $0.code.caseInsensitiveCompare(highlightedCode) == .orderedSame }) {
            commit(option: option)
            return
        }
